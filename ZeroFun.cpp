...
std::unique_ptr<SolverBase> SolverFactory::operator()(std::string solver_name) const
{
	if(solver_name == "Bisection")
		return std::make_unique<Bisection>(param.f, param.tol, param.maxIt,	param.interval,
										   param.maxIt_interval, param.h_interval);
	
	else if(solver_name == "RegulaFalsi")
		return std::make_unique<RegulaFalsi>(param.f, param.tol, param.maxIt, param.tola,
											 param.interval, param.maxIt_interval, param.h_interval);
		
	else if(solver_name == "Brent")
		return std::make_unique<Brent>(param.f, param.tol, param.maxIt,	param.interval,
									   param.maxIt_interval, param.h_interval);
			
	else if(solver_name == "Secant")
		return std::make_unique<Secant>(param.f, param.tol, param.maxIt, param.tola, param.interval);

	else if(solver_name == "Newton")
		return std::make_unique<Newton>(param.f, param.tol, param.maxIt, param.tola, param.x, param.df);
	
	else if(solver_name == "QuasiNewton")
		return std::make_unique<QuasiNewton>(param.f, param.tol, param.maxIt, param.tola, param.x, param.h);
...
void SolverFactory::set_param(Parameters & param_){
	
	param.f = param_.f;
	param.tol = param_.tol;
	param.maxIt = param_.maxIt;
	param.tola = param_.tola;
	param.x = param_.x;
	param.interval = param_.interval;
	param.h = param_.h;
	param.df = param_.df;
